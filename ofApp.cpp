...
void ofApp::setup() {
	...
	auto file_path = "D:\\MP4\\Pexels Videos 1333384.mp4";
...
void ofApp::update() {
	...
	for (int i = 0; i < this->draw_rect_size_list.size(); i++) {
		...
		this->draw_rect_size_list[i] = this->draw_rect_size_list[i] <= this->rect_size ? this->draw_rect_size_list[i] + 0.5 : this->rect_size;
	}
	...
	if (frame_index < next_frame_index) {
		...
		for (int i = 0; i < this->cv_rect_list.size(); i++) {
			...
			if ((gv[0] + gv[1] + gv[2]) > 100) {
				...
				this->draw_rect_size_list[i] = 0;

				this->moving_location_list.push_back(glm::vec3(this->cv_rect_list[i].x, this->cv_rect_list[i].y, 0));
				this->moving_frame_index_list.push_back(i);
			}
		}
	}
	...
	for (int i = this->moving_location_list.size() - 1; i >= 0; i--) {
		...
		if (this->moving_location_list[i].z > 200) {
			...
			this->moving_frame_index_list.erase(this->moving_frame_index_list.begin() + i);
		}
	}
}
...
void ofApp::draw() {
	...
	for (int i = 0; i < this->moving_location_list.size(); i++) {
		...
		ofPushMatrix();
		ofTranslate(this->moving_location_list[i].x + 50, this->moving_location_list[i].y + 180, this->moving_location_list[i].z);
		ofRotateX(180);

		this->rect_image_list[this->moving_frame_index_list[i]]->update();
		this->rect_image_list[this->moving_frame_index_list[i]]->getTexture().bind();

		ofSetColor(255);
		ofFill();
		ofDrawPlane(glm::vec3(), this->rect_size, this->rect_size);

		this->rect_image_list[this->moving_frame_index_list[i]]->unbind();
